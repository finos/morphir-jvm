...
package object sexpr {
  ...
  implicit class EncoderOps[A](private val a: A) extends AnyVal {
  ...
  implicit class DecoderOps(private val sexpr: CharSequence) extends AnyVal {
    def fromSExpr[A](implicit A: SExprDecoder[A]): Either[String, A] = A.decodeSExpr(sexpr)
  }

  implicit class SExprOps[A](private val sexpr: SExpr) extends AnyVal {
