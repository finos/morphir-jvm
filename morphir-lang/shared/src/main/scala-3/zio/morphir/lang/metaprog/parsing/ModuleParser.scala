...
object ModuleParser {
    ...
    object MorphirMethodDef:
      def unapply(using Quotes)(term: quotes.reflect.Statement): Option[(/*method name*/ String, /*variables*/ List[quotes.reflect.ValDef], /*body*/ quotes.reflect.Term, quotes.reflect.TypeTree)] = 
        import quotes.reflect._
        term match
          case 
            DefDef(
    ...
    def parseSpecAndDef[T:Type](body:Expr[T])(using Quotes): Expr[ModuleInfo[T]] = {
      ...
      deser.asTerm match {
        case
          ...
          methods.map {
            case MorphirMethodDef(name, vars, body, ret) =>
              println(s"=============== Matched Method ${name}! ==================")
              println(Printer.TreeStructure.show(body))

              vars.map {
                case MorphirMethodVarDef(name, tpeIdent) =>
