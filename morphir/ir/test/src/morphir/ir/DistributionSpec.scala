...
object DistributionSpec extends DefaultRunnableSpec {
    val distributionDecoder: Decoder[Distribution] = (c: io.circe.HCursor) => c.downField("distribution").as(Codec.decodeDistribution)
    val bufferResource: BufferedSource = Source.fromFile("C:\\Users\\Administrator\\IdeaProjects\\morphir-elm\\tests-integration\\reference-model\\morphir-ir.json")
