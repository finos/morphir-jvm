...
object Type {
  ...
  val CustomTypeDefinition: morphir.ir.Type.Definition.CustomTypeDefinition.type =
    morphir.ir.Type.Definition.CustomTypeDefinition

  val TypeAliasDefinition: morphir.ir.Type.Definition.TypeAliasDefinition.type =
    morphir.ir.Type.Definition.TypeAliasDefinition
  ...
  type ResultList[E, A] = morphir.sdk.List.List[morphir.sdk.Result.Result[E, A]]
  ...
  val CustomTypeSpecification: morphir.ir.Type.Specification.CustomTypeSpecification.type =
    morphir.ir.Type.Specification.CustomTypeSpecification

  val DerivedTypeSpecification: morphir.ir.Type.Specification.DerivedTypeSpecification.type =
    morphir.ir.Type.Specification.DerivedTypeSpecification

  val OpaqueTypeSpecification: morphir.ir.Type.Specification.OpaqueTypeSpecification.type =
    morphir.ir.Type.Specification.OpaqueTypeSpecification

  val TypeAliasSpecification: morphir.ir.Type.Specification.TypeAliasSpecification.type =
    morphir.ir.Type.Specification.TypeAliasSpecification
  ...
  def collectReferences[Ta](
    ...
    def collectUnion(
      ...
      morphir.sdk.List.foldl(morphir.sdk.Set.union[FQName])(morphir.sdk.Set.empty)(
        morphir.sdk.List.map(morphir.ir.Type.collectReferences[Ta])(values)
      )
    ...
    tpe match {
      ...
      case morphir.ir.Type.Record(_, fields) =>
        collectUnion(morphir.sdk.List.map(((x: morphir.ir.Type.Field[Ta]) => x.tpe))(fields))
      ...
      case morphir.ir.Type.CustomTypeDefinition(_, accessControlledType) =>
        morphir.sdk.List.foldl(morphir.sdk.Set.union[FQName])(morphir.sdk.Set.empty)(
          morphir.sdk.List.map(
            morphir.sdk.Basics
              .composeRight(morphir.sdk.Tuple.second[Name, Type[Ta]])(morphir.ir.Type.collectReferences[Ta])
          )(morphir.sdk.List.concat(morphir.sdk.Dict.values(accessControlledType.value)))
        )
    }
  ...
  def collectVariables[Ta](
    ...
    def collectUnion(
      ...
      morphir.sdk.List.foldl(morphir.sdk.Set.union[Name])(morphir.sdk.Set.empty[Name])(
        morphir.sdk.List.map(morphir.ir.Type.collectVariables[Ta])(values)
      )
    ...
    tpe match {
      ...
      case morphir.ir.Type.ExtensibleRecord(_, subjectName, fields) =>
        morphir.sdk.Set.insert(subjectName)(
          collectUnion(morphir.sdk.List.map(((x: morphir.ir.Type.Field[Ta]) => x.tpe))(fields))
        )
      case morphir.ir.Type.Function(_, argType, returnType) =>
        collectUnion(
    ...
    typeDef match {
      ...
      case morphir.ir.Type.CustomTypeDefinition(typeVars, acsCtrlConstructors) => {
        def eraseCtor(
          ...
          morphir.sdk.List.map(
            ({ case (n, t) =>
              ...
              (morphir.ir.Name.Name, morphir.ir.Type.Type[A])
            ) => (morphir.ir.Name.Name, morphir.ir.Type.Type[scala.Unit]))
          )(types)
        ...
        def eraseAccessControlledCtors(
          ...
          morphir.ir.AccessControlled.map(
            (
              (ctors: morphir.sdk.Dict.Dict[morphir.ir.Name.Name, morphir.ir.Type.ConstructorArgs[A]]) =>
                morphir.sdk.Dict.map(
                  ({ case _ =>
                    eraseCtor
                  }: morphir.ir.Name.Name => morphir.ir.Type.ConstructorArgs[A] => morphir.ir.Type.ConstructorArgs[
                    scala.Unit
                  ])
                )(ctors)
            )
  ...
  def mapDefinition[A, B, E](
    fn: morphir.ir.Type.Type[A] => morphir.sdk.Result.Result[E, morphir.ir.Type.Type[B]]
  )(
    valueDef: morphir.ir.Type.Definition[A]
  ): morphir.sdk.Result.Result[morphir.sdk.List.List[E], morphir.ir.Type.Definition[B]] =
    valueDef match {
      case Definition.TypeAliasDefinition(params, tpe) =>
        val res = Result.map((mappedTpe: Type[B]) => TypeAliasDefinition(params, mappedTpe))(fn(tpe))
        Result.mapError((err: E) => morphir.sdk.List.singleton(err))(res)
      case Definition.CustomTypeDefinition(params, constructors) => {
        import scala.util.chaining._
        import morphir.sdk.List
        val ctorsResult: Result.Result[List.List[E], AccessControlled[Constructors[B]]] = {
          constructors.value
            .pipe(Dict.toList[Name, ConstructorArgs[A]])
            .pipe(List.map { case (ctorName: Name, ctorArgs: ConstructorArgs[A]) =>
              ctorArgs
                .pipe(List.map { case (argName, argType) =>
                  fn(argType)
                    .pipe(Result.map((mappedType: Type[B]) => (argName, mappedType)))
                })
                .pipe(keepAllErrors)
                .pipe(Result.map((v: List.List[(Name, Type[B])]) => (ctorName, v)))

            })
            .pipe(keepAllErrors[(Name, List.List[(Name, Type[B])]), List.List[E]])
            .pipe(
              Result.map[List[List.List[E]], List.List[(Name, List.List[(Name, Type[B])])], AccessControlled[
                Constructors[B]
              ]]((listOfPairs: List.List[(Name, List.List[(Name, Type[B])])]) =>
                morphir.ir.AccessControlled.AccessControlled(
                  constructors.access,
                  Dict.fromList[Name, List.List[(Name, Type[B])]](listOfPairs)
                )
              )
            )
            .pipe(Result.mapError(List.concat))
        }

        ctorsResult
          .pipe(Result.map(CustomTypeDefinition(params, _)))
      }
    }
                  ...
                  (ctorArgs: morphir.sdk.List.List[(morphir.ir.Name.Name, morphir.ir.Type.Type[A])]) =>
                    morphir.sdk.List.map(
                      ({ case (argName, argType) =>
                        ...
                        (morphir.ir.Name.Name, morphir.ir.Type.Type[A])
                      ) => (morphir.ir.Name.Name, morphir.ir.Type.Type[B]))
                    )(ctorArgs)
                )
              }: morphir.ir.Name.Name => morphir.sdk.List.List[
                (morphir.ir.Name.Name, morphir.ir.Type.Type[A])
              ] => morphir.sdk.List.List[(morphir.ir.Name.Name, morphir.ir.Type.Type[B])])
            )(constructors.value)
          ): morphir.ir.AccessControlled.AccessControlled[morphir.sdk.Dict.Dict[
  ...
  def _toString[A](
    ...
    tpe match {
      ...
      case morphir.ir.Type.Reference(_, (packageName, moduleName, localName), args) => {
        val referenceName: morphir.sdk.String.String = morphir.sdk.String.join(".")(
          morphir.sdk.List(
        ...
        morphir.sdk.String.join(" ")(
          morphir.sdk.List.cons(referenceName)(morphir.sdk.List.map(morphir.ir.Type._toString[A])(args))
        )
      }
