...
object Encode {
  ...
  def int(value: Int): Value = ujson.Num(value.toDouble)

  def long(value: Long): Value = ujson.Num(value.toDouble)

  def float(value: Float): Value = ujson.Num(value.toDouble)
  ...
  def dict[K, V](keyMap: K => String): (V => Value) => Map[K, V] => Obj =
    (valueMap: V => Value) =>
      (props: Map[K, V]) =>
        ujson.Obj.from(
          props
