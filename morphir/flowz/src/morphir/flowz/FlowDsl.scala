...
trait FlowDsl {
  ...
  object Flow {
    ...
    sealed trait Builder[-StartupEnv, -Input, Env, State, Params, +Output, Phase] { self =>
      ...
      )(implicit ev: Any <:< Phase): Builder[StartupEnv, Input1, Env1, State1, Params1, Output, Phase with Phases.Setup]
      ...
      ): Builder[StartupEnv, Input, Env, State, Params, Output1, Phase with Phases.DefineStages]
      ...
      ): Builder[StartupEnv, Input, Env, State, Params, Output, Phase with Phases.Report]
    }
    ...
    object Builder {
      ...
      private sealed case class FlowBuilder[StartupEnv, Input, Env, StateIn, Params, Output, Phase](
        ...
        output: Option[Output => ZIO[Env, Throwable, Any]]
        ...
        def build(implicit
          ...
          new Flow[StartupEnv, Input, Output] {
            ...
            def step: Step[InitialState, _, Env, Params, Throwable, Output] = self.step.get.tap { case (_, output) =>
              self.output.fold[ZIO[Env, Throwable, Any]](ZIO.unit)(_(output))
            }
          }
        }
        ...
        ): Builder[StartupEnv, Input, Env, StateIn, Params, Output1, Phase with Phases.DefineStages] =
          FlowBuilder[StartupEnv, Input, Env, StateIn, Params, Output1, Phase with Phases.DefineStages](
            name = self.name,
        ...
        def report(
          f: Output => ZIO[Env, Throwable, Any]
        ): Builder[StartupEnv, Input, Env, StateIn, Params, Output, Phase with Phases.Report] =
          FlowBuilder[StartupEnv, Input, Env, StateIn, Params, Output, Phase with Phases.Report](
            name = self.name,
            step = self.step,
            setupEffect = self.setupEffect,
            output = self.output.map(g => (o: Output) => f(o) *> g(o)) orElse Some(f)
          )
