...
object Type {
    ...
    def variable[A](name:Name)(annotations:A):Type[A] = Type(annotations, TypeDetails.Variable(name))
}
...
enum TypeDetails[+A]: 
    ...
    def transform[B](f:A=>B):TypeDetails[B] = self match
        ...
        case Function(parameters, returnType) =>
            Function(
                parameters = parameters.transform(f), 
                returnType = returnType.transform(f))
        case Unit => Unit
